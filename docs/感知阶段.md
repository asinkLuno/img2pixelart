...
```python
denoised = cv2.bilateralFilter(bgr, d=5, sigmaColor=35, sigmaSpace=35)
blocks = cv2.pyrMeanShiftFiltering(denoised, sp=8, sr=25)
```
...
| 参数 | 默认值 | 影响 |
|------|--------|------|
| `denoise_d` | 5 | 双边滤波核直径，奇数 |
| `denoise_sigma` | 35 | 双边滤波颜色敏感度。↑ 保留更多细节 |
