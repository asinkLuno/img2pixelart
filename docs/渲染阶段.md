...
*硬分档着色（size=96）。每个像素精确取自色板的某一档，无任何过渡。*
...
*抖动掩码（pattern 方法，size=96）。只有渐变区域的像素参与抖动。*
...
```
final[y,x] = ramps[family[y,x], silhouette_dark_step]
```
...
```
final[y,x] = ramps[family[y,x], current_tier - internal_outline_dark_steps]
```
...
| `internal_outline_dark_steps` | 1 | 内部线暗化档数。↑ 细节越深 |
